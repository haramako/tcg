...
namespace :lint do
  desc 'マークされた文字列を検出'
  task :mark do
    sh 'ruby', 'Tools/RubyLib/mark_detector.rb', 'TcgClient/Assets'
  end
...
task :proto do
  ...
  mkdir_p "#{PROJECT_ROOT}/TcgClient/Assets/Scripts/Tcg/AutoGenerated"
  ...
  sh PROTOC,
     ...
     "--dfcsharp_out=#{PROJECT_ROOT}/TcgClient/Assets/Scripts/Tcg/AutoGenerated/",
     "--proto_path=#{PROTO_DIR}",
     *FileList[PROTO_DIR + '*.proto']
  FileList["#{PROJECT_ROOT}/Tools/RubyLib/autogen/*.rb", "#{PROJECT_ROOT}/TcgClient/Assets/Scripts/Tcg/AutoGenerated/*.cs"].each do |f|
    src = IO.binread(f)
    IO.write(f, src)
  end
end
...
task :codegen do
  sh "ruby #{PROJECT_ROOT}/Tools/CodeGen/codegen #{PROJECT_ROOT}/Tools/CodeGen/global_cabinet.rb >#{PROJECT_ROOT}/TcgClient/Assets/Scripts/Tcg/AutoGenerated/Cabinet.cs"
  sh "ruby #{PROJECT_ROOT}/Tools/CodeGen/codegen #{PROJECT_ROOT}/Tools/CodeGen/game_cabinet.rb >#{PROJECT_ROOT}/TcgClient/Assets/Scripts/Tcg/AutoGenerated/GameCabinet.cs"
end

desc 'ソースコードの行数を取得する'
task :src_lines do
  cs = FileList['TcgClient/Assets/**/*.cs'].to_a
  cs = cs
       .reject { |f| f =~ %r{/AutoGenerated/} }
       .reject { |f| f =~ %r{/Vendor/} }
       .reject { |f| f =~ %r{/Utage/} }
